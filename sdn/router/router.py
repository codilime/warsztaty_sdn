...
class Router(object):
    ...
    @staticmethod
    def _build_unbind_command(iface_in, iface_out):
        return ['iptables', '-D', 'FORWARD', '-i', iface_in, '-o', iface_out, '-j', 'ACCEPT']
    ...
    def remove_logical_port(self, net, ip):
        ...
        for peer in self.networks[net]:
            ...
            self.command_executor.execute(self._build_unbind_command(peer_interface, my_interface))
            self.command_executor.execute(self._build_unbind_command(my_interface, peer_interface))
